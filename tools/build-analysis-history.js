...
const sources = [
  {
    ...
    summary:
      "依 Kevin 指正，修正選題邏輯為先盤點知識庫低頻主題；本輪選出健保欠費與保單借款/解約金兩個0命中主題，查核台灣官方資料後生成一般民眾純文字試產稿，並二次加入財務風險判斷與解法選擇 gate。",
    highlights: [
      "不沿用前一輪被資遣與房租現金流兩題。",
      "健保欠費題把就醫權、欠費分期、30天生活費與求助窗口分開。",
      "保單借款題把急用錢、利息、保單效力、理賠扣除與解約後保障缺口分開。",
      "新增 financialRiskDecisionReview：文章必須讓讀者判斷風險、理解家庭財務後果、選擇符合台灣現況且不牽強的解法。",
      "三張通過稿結構卡初步支持此方向：通過稿多有前後差異、制度判斷順序、風險分層或正式求助分流；目前仍標記為小樣本質性驗證。"
    ]
  },
  {
    ...
    type: "投稿後退件學習",
    title: "結構化試產稿投稿後退件，新增現金流與高變異 gate",
    summary:
      "兩篇 6/17 結構化試產稿投稿後再次被總編退件。現金流篇學到重點不是敘事或表格，而是結構性盤點財務及現金流；資遣篇學到高變異條件題材不能靠個案情境作主要參考，應改成變數清單、決策表或工具型內容。",
    highlights: [
      "新增 `structuralCashflowReview`：房租、扣款、薪水、補貼與月底缺口題材，必須帶讀者完成收入時間、固定承諾、扣款日、受保護金額、可用現金、缺口類型與下一步。",
      "新增 `highVariationTopicReview`：資遣、失業給付、補助、債務、長照、保險與地方資源題材，若個別條件會大幅改變答案，不得以個案情境當主要參考。",
      "此包已標記為投稿後退件學習，不建議直接投稿，也不得作為成功稿學習樣本。"
    ]
  },
  {
